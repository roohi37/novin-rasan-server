...
func (c *session) sendPushToQueue(ctx context.Context, gatewayId string, pushMsgId int64, pushMsg mtproto.TLObject) {
	...
	// Novin Rasan: if no live session can receive this push, persist it so it
	// can be replayed when the user reconnects (offline push).
	c.saveOfflinePushes(ctx, rawBytes)
...
// saveOfflinePushes persists a push to the push_queue table when the user has
// no online session that can receive it right now. Skips the androidPush
// pseudo-session so we do not queue our own internal keepalive pushes.
func (c *session) saveOfflinePushes(ctx context.Context, rawBytes []byte) {
	logx.WithContext(ctx).Infof("saveOfflinePushes: ENTER isAndroidPush=%v len=%d authUserId=%d", c.isAndroidPush, len(rawBytes), c.sessList.cb.AuthUserId)
	if c.isAndroidPush || len(rawBytes) == 0 {
		logx.WithContext(ctx).Infof("saveOfflinePushes: SKIP guard1 isAndroidPush=%v len=%d", c.isAndroidPush, len(rawBytes))
		return
	}
	cb := c.sessList.cb
	if cb == nil || cb.AuthUserId <= 0 {
		logx.WithContext(ctx).Infof("saveOfflinePushes: SKIP guard2 cb_nil=%v", cb == nil)
		return
	}
	if cb.isAnySessionOnline() {
		logx.WithContext(ctx).Infof("saveOfflinePushes: SKIP guard3 user_online authUserId=%d", cb.AuthUserId)
		// a live session will deliver it synchronously; nothing to queue
		return
	}
	dao2 := cb.cb.Dao
	if dao2 == nil || dao2.PushQueue == nil {
		logx.WithContext(ctx).Infof("saveOfflinePushes: SKIP guard4 dao_nil=%v", dao2 == nil)
		return
	}
	if err := dao2.PushQueue.SaveOfflinePush(
		ctx,
		cb.AuthUserId,
		c.sessList.authId,
		c.sessionId,
		...
		logx.WithContext(ctx).Infof("saveOfflinePushes: queued push for user_id: %d, auth_key_id: %d, session_id: %d, len: %d",
			cb.AuthUserId, c.sessList.authId, c.sessionId, len(rawBytes))
	} else {
		logx.WithContext(ctx).Errorf("saveOfflinePushes: SAVE ERR: %v", err)
	}
}
