...
type SessionList struct {
	...
	// saltMu guards lazy/refresh of cacheSalt so concurrent requests don't
	// each fetch a different future_salt (salt-race -> bad_server_salt loop).
	saltMu        sync.Mutex
...
// IsOnline reports whether any real session in this list currently has a live
// connection. Used by the offline-push queue to decide whether a push can be
// delivered synchronously or must be persisted for later.
func (s *SessionList) IsOnline() bool {
	for _, sess := range s.sessions {
		if sess != nil && sess.isAndroidPush {
			continue
		}
		if sess != nil && sess.sessionOnline() {
			return true
		}
	}
	return false
...
func (m *MainAuthWrapper) setOnline(ctx context.Context) {
	...
	if (m.onlineExpired == 0 || date > m.onlineExpired-kPingAddTimeout) && m.AuthUserId != 0 {
		...
		// Novin Rasan: on (re)connect, replay any pushes that were queued while
		// the user was offline. Best-effort; failures must not block online.
		if m.cb != nil && m.cb.Dao != nil && m.cb.Dao.PushQueue != nil && m.cb.Dao.PushQueue.Enabled() {
			go m.replayOfflinePushes()
		}
	} else {
...
// isAnySessionOnline reports whether the user has a real (non-android-push)
// session with a live connection able to receive a push synchronously.
// The androidPush pseudo-session is deliberately excluded: it can stay
// "online" while the actual client is offline, and we must still queue the
// push in that case.
func (m *MainAuthWrapper) isAnySessionOnline() bool {
	if m.mainAuth != nil && m.mainAuth.IsOnline() {
		return true
	}
...
// replayOfflinePushes re-delivers pushes that were queued while the user was
// offline. Runs in a goroutine from setOnline; best-effort.
func (m *MainAuthWrapper) replayOfflinePushes() {
	if m.cb == nil || m.cb.Dao == nil || m.cb.Dao.PushQueue == nil || !m.cb.Dao.PushQueue.Enabled() {
		return
	}
	ctx := context.Background()
	rows, err := m.cb.Dao.PushQueue.FetchPending(ctx, m.AuthUserId, 100)
	if err != nil {
		logx.WithContext(ctx).Errorf("replayOfflinePushes: fetch error: %v", err)
		return
	}
	for _, p := range rows {
		obj := mtproto.NewDecodeBuf(p.Data).Object()
		var upds *mtproto.Updates
		switch v := obj.(type) {
		...
		if upds == nil {
			logx.WithContext(ctx).Infof("replayOfflinePushes: skip non-updates id=%d", p.Id)
			_ = m.cb.Dao.PushQueue.MarkDelivered(ctx, []int64{p.Id})
			continue
		}
		if err = m.SyncDataArrived(ctx, false, upds); err == nil {
			_ = m.cb.Dao.PushQueue.MarkDelivered(ctx, []int64{p.Id})
			logx.WithContext(ctx).Infof("replayOfflinePushes: replayed id=%d for user_id=%d", p.Id, m.AuthUserId)
		}
	}
}

func (m *MainAuthWrapper) trySetOffline(ctx context.Context) {
	if m.isAnySessionOnline() {
		return
	}
...
// ensureSalt lazily loads or refreshes cacheSalt/cacheLastSalt for this auth
// session under saltMu, so concurrent requests observe the SAME salt. Without
// this, each goroutine fetched its own future_salt and the client's salt never
// matched cacheSalt -> infinite bad_server_salt loop (login never completes).
func (sList *SessionList) ensureSalt(ctx context.Context, m *MainAuthWrapper) {
	sList.saltMu.Lock()
	defer sList.saltMu.Unlock()
...
func (m *MainAuthWrapper) onSessionData(ctx context.Context, sessionMsg *sessionData) {
	...
	// Load or refresh salt (single-flight under saltMu).
	sList.ensureSalt(ctx, m)
