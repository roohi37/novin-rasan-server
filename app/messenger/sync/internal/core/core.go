...
func (c *SyncCore) pushUpdatesToSession(syncType SyncType, userId, permAuthKeyId int64, hasServerId *wrapperspb.StringValue, authKeyId, sessionId *wrapperspb.Int64Value, pushData *mtproto.Updates, notification bool) {
	if syncType == syncTypeUserMe && hasServerId != nil {
		...
		// Novin Rasan: when the target user has no online session, persist the
		// push so it can be replayed on reconnect (offline push queue).
		if len(serverIdKeyIdList) == 0 && c.svcCtx.Dao.PushQueue != nil && c.svcCtx.Dao.PushQueue.Enabled() {
			x := mtproto.GetEncodeBuf()
			if err := pushData.Encode(x, 228); err == nil {
				raw := append([]byte(nil), x.GetBuf()...)
				mtproto.PutEncodeBuf(x)
				_ = c.svcCtx.Dao.PushQueue.SaveOfflinePush(c.ctx, userId, permAuthKeyId, 0, 0, raw)
				c.Logger.Infof("pushUpdatesToSession: queued offline push for user_id=%d, len=%d", userId, len(raw))
			} else {
				mtproto.PutEncodeBuf(x)
				c.Logger.Errorf("pushUpdatesToSession: encode error: %v", err)
			}
		}
