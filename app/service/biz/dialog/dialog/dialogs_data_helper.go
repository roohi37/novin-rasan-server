...
// isUsablePeer reports whether p carries one of the three peer predicates that
// mtproto.FromPeer accepts. Anything else (nil or an unset/unknown predicate)
// would make FromPeer panic, so callers must skip such records instead.
func isUsablePeer(p *mtproto.Peer) bool {
	if p == nil {
		return false
	}
	switch p.GetPredicateName() {
	case mtproto.Predicate_peerUser, mtproto.Predicate_peerChat, mtproto.Predicate_peerChannel:
		return true
	}
	return false
}
...
func (m *DialogsDataHelper) fix() {
	...
	for _, dlg := range m.Dialogs {
		...
		// A corrupted/unmatched dialog peer must never make FromPeer panic;
		// skip the whole dialog instead.
		if !isUsablePeer(dlg.Peer) {
			continue
		}
		p := mtproto.FromPeer(dlg.Peer)
		for _, msg := range m.Messages {
			if !isUsablePeer(msg.PeerId) {
				continue
			}
