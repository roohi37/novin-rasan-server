...
// isUsableDialogPeer reports whether peer carries one of the three predicates
// mtproto.FromPeer accepts; anything else would panic FromPeer, so skip it.
func isUsableDialogPeer(peer *mtproto.Peer) bool {
	if peer == nil {
		return false
	}
...
func (c *DialogsCore) MessagesGetDialogs(in *mtproto.TLMessagesGetDialogs) (*mtproto.Messages_Dialogs, error) {
	...
	for _, dialogEx := range dialogExtList {
		peer := dialogEx.GetDialog().GetPeer()
		if !isUsableDialogPeer(peer) {
			// A corrupted dialog peer must never panic FromPeer; skip it.
			continue
		}
		peer2 := mtproto.FromPeer(peer)
