...
func (c *NotificationCore) AccountUpdateNotifySettings(in *mtproto.TLAccountUpdateNotifySettings) (*mtproto.Bool, error) {
	...
	case mtproto.PEER_CHANNEL:
		if c.svcCtx.Plugin != nil {
			...
			// نوین رسان: سرویس Channel در متن‌باز نیست؛ به‌جای بلوکه‌کردن،
			// تنظیم اعلان را با peerType/peerId ثبت می‌کنیم (UserSetNotifySettings
			// به peerChannel نیاز ندارد).
			c.Logger.Infof("account.updateNotifySettings: channel %d notify settings stored (no Channel service)", peer.PeerId)
		}
	default:
		err = mtproto.ErrPeerIdInvalid
		c.Logger.Errorf("account.updateNotifySettings - error: %v", err)
