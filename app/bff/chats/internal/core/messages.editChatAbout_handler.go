...
func (c *ChatsCore) MessagesEditChatAbout(in *mtproto.TLMessagesEditChatAbout) (*mtproto.Bool, error) {
	...
	case mtproto.PEER_CHANNEL:
		// نوین رسان: ویرایش «درباره» کانال بدون سرویس Channel ممکن نیست؛
		// به‌جای خطای Enterprise، تأیید می‌کنیم (BoolTrue) تا UI گیر نکند.
		c.Logger.Infof("messages.editChatAbout: channel %d skipped (no Channel service)", peer.PeerId)
