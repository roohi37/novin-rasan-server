...
func (c *DraftsCore) MessagesSaveDraft(in *mtproto.TLMessagesSaveDraft) (*mtproto.Bool, error) {
	...
	case mtproto.PEER_CHANNEL:
		if c.svcCtx.Plugin != nil {
			...
			// نوین رسان: سرویس Channel در متن‌باز نیست؛ پیش‌نویس ذخیره شده را
			// از دست نمی‌دهیم و فقط chat metadata را جا می‌گذاریم.
			c.Logger.Infof("messages.saveDraft: draft stored for channel %d (no Channel service)", peer.PeerId)
		}
	}
