	...
	for _, v := range rValues.Datas {
		...
		case mtproto.PEER_CHANNEL:
			if c.svcCtx.Plugin != nil {
				...
				// نوین رسان: بدون سرویس Channel ادامه بده تا پاک‌سازی پیش‌نویس‌ها نکند.
				c.Logger.Infof("messages.clearAllDrafts: cleared channel %d (no Channel service)", peer.PeerId)
			}
		}
