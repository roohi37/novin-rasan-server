...
func (c *MessagesCore) MessagesSearch(in *mtproto.TLMessagesSearch) (*mtproto.Messages_Messages, error) {
	...
	if peer.IsChannel() {
		// نوین رسان: سرویس Channel در متن‌باز نیست. چون دادهٔ کانال در همین
		// نسخه در جدول مشترک messages/dialogs نگه‌داری می‌شود، جستجو را روی
		// همان دیالوگ انجام می‌دهیم (بدون خطای Enterprise).
		c.Logger.Infof("messages.search: channel %d searched via shared message store", peer.PeerId)
	}
