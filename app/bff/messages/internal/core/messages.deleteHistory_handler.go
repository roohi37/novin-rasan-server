...
func (c *MessagesCore) MessagesDeleteHistory(in *mtproto.TLMessagesDeleteHistory) (*mtproto.Messages_AffectedHistory, error) {
	...
	if peer.IsChannel() {
		// نوین رسان: بدون سرویس Channel؛ حذف تاریخچه کانال را با همان
		// مسیر دیالوگِ مشترک انجام می‌دهیم.
		c.Logger.Infof("messages.deleteHistory: channel %d via shared dialog store", peer.PeerId)
	}
